"""
step3.2_forecast_2026.py — Q3 2026 ensemble forecast with uncertainty
=====================================================================
Loads trained models -> FILT reconstruction -> 2h ensemble -> 1h interpolation
"""
...
def main():
    ...
    all_rows = []
    for dl, dt, init_src in [("Feb1", "2026-02-01", f"Jan_mean={jan_ntu_mean:.3f}"),
                               ("Feb10", "2026-02-10", "2025-02-10_NTU_mean"),
                               ("Feb20", "2026-02-20", "2025-02-20_NTU_mean")]:
        f1h = reconstruct_filt(feb_df, dl)
        f2h_vals = np.array([f1h[Q_HOURS.index(h)] for h in HOURS_2H])
        init = jan_ntu_mean if dl == "Feb1" else (
