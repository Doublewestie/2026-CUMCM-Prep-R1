...
| Q3 | 6-12h出厂NTU逐小时预测 | 预测类(时序) + 条件路由 | 1:00-5:00已知数据 + CSTR链 + 天型分类器 | 2026年3天7:00-19:00逐小时NTU, 消融表 |
...
| $\alpha_B$ | Type B CSTR混合权重 | 固定参数 | — | 0.34 | Q3全局扫描 |
| $\gamma_W$ | C_weak CSTR阻尼因子 | 固定参数 | — | 0.25 | Q3全局扫描 |
| $C_{th}$ | CSTR适用阈值 | 固定参数 | NTU | 1.0 | Q3全局扫描 |
...
> **【已废止】** 以下 3.1-3.4 内容为原始 TCN+注意力+PINN 方案的完整推导，保留备查。
> 实际执行方案：log-AR(6)+RidgeCV, 见 `step2.5_logar_final.py` + `docs/sums/sum_8.md`。
> 最终 CV R²=**0.696** (5-fold TS-CV)。五重证据链证明闭环掩蔽效应 (see `Reference/sums/sum_10.md`)。
...
**【方法变更 2026-07-26】** 原方案(TCN→GRU+N-BEATS+RF元学习器, Section 4.1-4.4)已替换为四层条件路由。详见 `docs/sums/sum_12_Q3四层条件路由最终方案.md` 和 `step3.8_final_stratified.py`。

**最终方案概要**：
```
预测日 5:00 → 四层分类 → 分策略预测
  A (稳态):  NTU(h) = NTU(5:00)
  B (过渡):  NTU(h) = 0.34·CSTR(h) + 0.66·NTU(5:00)
  C_strong:  NTU(h) = CSTR(h)           (FILT ≥ 1.0)
...
### 4.3 公式推导（原方案，已废止，保留备查）

> **【以下 4.3-4.4 的全部数学推导 (TCN→GRU, N-BEATS, CSTR卷积, 门控机制, 元特征矩阵, RF元学习器, Sobol, 统一Loss) 已废止。**
> 新方案为四层条件路由 (A/B/C_strong/C_weak)，参数 α=0.34, γ=0.25, C_th=1.0, CV R²=0.602。
> 详见 `step3.8_final_stratified.py` + `docs/sums/sum_12_Q3四层条件路由最终方案.md`。

#### 4.3.1 源A: TCN→GRU集成模型（废止）
...
> **【已废止】** 以下 6.1-6.4 是基于已废止 Q3 双源架构的推理策略，保留备查。
> **当前方案 (2026-07-26)**: 2026年的预测待数据适配完成后，将使用 2025 年同日期数据作为 FILT/CW/Q/RL 代理，结合 Q3 四层条件路由预测。当前 `step3.8_final_stratified.py` Part C 框架已就位。

### 6.1 数据格式差异的根本原因分析（废止）
...
> **【已废止】** 以下 6.1-6.2 的 Dual-Source Unified Loss 设计基于已废止的 Q3 双源架构。

### 当前实际依赖 (2026-07-26):
```
Q1 (CSTR公式+A参数+平衡检测器) → Q3 (继承CSTR链)
Q2 (log-AR(6)+时滞参数+闭环掩蔽) → Q3 (方法论参考, 不直接输入)
Q3 (四层条件路由) → Q4 (风险评分)
```
...
| 2026-07-23 23:00 | 补充三处缺口: §4.3.3 CSTR级数, §4.3.4门控机制, §6推理策略 |
| 2026-07-26 18:00 | **全面审视更新**: Q1/Q2/Q3所有废止节加注; Q3 §4替换为四层条件路由概要; §7统一Loss废止; 附录新增当前全局参数; DAG更新; meta更新至version 2.0 |

---

## 附录F: 当前全局参数 (2026-07-26 终版)

### Q1 CSTR参数 (step1.7_final_cstr.py)
| 参数 | 值 | 说明 |
|------|:---:|------|
| A_T1 | 400 | T1 (FILT≤0.05) |
...
| 参数 | 值 | 说明 |
|------|:---:|------|
| 模型 | log(FILT+1e-3) AR(6)+RidgeCV | 7参数 |
| CV R² | 0.696 | 5-fold TS-CV |

### Q3 四层条件路由参数 (step3.8_final_stratified.py)
| 参数 | 值 | 说明 |
|------|:---:|------|
